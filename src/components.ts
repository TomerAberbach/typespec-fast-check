...
const GlobalArbitraryNamespace = stc(
  ({
    namespace,
    sharedArbitraries,
  }: {
          ...
          namespace =>
            Commented({ comment: namespace.comment }).children(
              ts.VarDeclaration({
                ...
                const: true,
                name: namespace.name,
                value: NestedArbitraryNamespace({
                  namespace,
                  sharedArbitraries,
                }),
              }),
...
const NestedArbitraryNamespace = stc(
  ({
    namespace,
    sharedArbitraries,
  }: {
...
const ConstantArbitrary = stc(
  ({ arbitrary }: { arbitrary: ConstantArbitrary }): Child =>
    CallExpression({
      name: `fc.constant`,
...
const BooleanArbitrary = stc((): Child => code`fc.boolean()`)

const NumberArbitrary = stc(
  ({ arbitrary }: { arbitrary: NumberArbitrary }): Child => {
    ...
    if (arbitraryMin !== null && min.normalize) {
      arbitraryMin = {
        ...arbitraryMin,
        value: min.normalize(arbitraryMin.value),
      }
    }
    ...
    if (arbitraryMax !== null && max.normalize) {
      arbitraryMax = {
        ...arbitraryMax,
        value: max.normalize(arbitraryMax.value),
      }
    }
...
const BigIntArbitrary = stc(
  ({ arbitrary }: { arbitrary: BigIntArbitrary }): Child =>
    CallExpression({
      name: `fc.bigInt`,
      args: [
        ObjectExpression({
...
const StringArbitrary = stc(
  ({ arbitrary }: { arbitrary: StringArbitrary }): Child =>
    CallExpression(
...
const UrlArbitrary = stc((): Child => code`fc.webUrl()`)

const BytesArbitrary = stc((): Child => code`fc.uint8Array()`)

const EnumArbitrary = stc(
  ({ arbitrary }: { arbitrary: EnumArbitrary }): Child =>
    CallExpression({
  ...
  ({
    ...
    CallExpression({
      ...
      args: [
        ...
        arbitrary.minItems !== undefined || arbitrary.maxItems !== undefined
          ? ObjectExpression({
              properties: {
...
const IntersectionArbitrary = stc(
  ({
    arbitrary,
    sharedArbitraries,
    currentStronglyConnectedArbitraries,
...
const RecursiveReferenceArbitrary = stc(
  ({ arbitrary }: { arbitrary: ReferenceArbitrary }): Child =>
    code`tie(${StringLiteral({ string: arbitrary.name })})`,
)

const FunctionDeclarationArbitrary = stc(
  ({
    arbitrary,
    sharedArbitraries,
    currentStronglyConnectedArbitraries,
...
const FunctionCallArbitrary = stc(
  ({
    arbitrary,
    sharedArbitraries,
    currentStronglyConnectedArbitraries,
...
const ArrayExpression = stc(
  ({ values }: { values: Child[] }): Child =>
    code`[${ayJoin(values, { joiner: `, ` })}]`,
)

const ObjectExpression = stc(
  ({
    properties,
    emitEmpty = false,
...
const ObjectProperty = stc(
  ({ name, value }: { name: string; value: Child }): Child =>
    memo(() => {
      ...
      return name ===
        (isRefkey(value) ? resolve(value).value.targetDeclaration.name : value)
        ? name
        : code`${name}: ${value}`
    }),
)
...
const Comment = stc(({ comment }: { comment: string }): Child => {
...
const StringLiteral = stc(
  ({ string }: { string: string }): Child => JSON.stringify(string),
)
